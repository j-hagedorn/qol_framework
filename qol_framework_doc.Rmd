...
A framework which seeks to be as broad as the one we are proposing runs the risk of becoming overwhelming complex and unwieldy.  So, before diving into any details, we want to begin by sketching out a simple way to think about the scope of the measurement framework using three-dimensions (*[depth](#zoom), [breadth](#pop), and [height](#steps)*)^[Those not geometrically inclined might want to think of "*the depth and breadth and height*" from [this sonnet](https://www.poetryfoundation.org/poems/43742/sonnets-from-the-portuguese-43-how-do-i-love-thee-let-me-count-the-ways)] defined below:
...
If we think about the framework as a cube made of these three dimensions, then developing the measurement framework can begin by making certain that data exists:

- from each step of the person-centered PDCA process (*height*) 
...
The next sections define each of the dimensions listed above, and how they relate to one another, in greater detail.  Please note that this paper does not develop or identify specific measures, except as illustrations of how individual data elements or metrics *might* fit into the overall framework.
...
knitr::kable(caption = 'Need Assessment and QoL Dimensions')
```
